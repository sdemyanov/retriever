...
def container_email_documents_missing_threading(
    ...
    *,
    ...
    root_document_ids = sorted(
        container_root_document_ids_for_source(
            connection,
            source_kind=source_kind,
            source_rel_path=source_rel_path,
        )
...
def pst_chat_cluster_scope_key(document: dict[str, object]) -> tuple[str, str]:
    ...
    if thread_id:
        return (filesystem_dataset_locator(), f"thread:{thread_id.lower()}")
...
def pst_chat_conversation_source_locator(cluster: dict[str, object]) -> str:
    if any(normalize_pst_chat_thread_id(value) for value in cluster["thread_ids"]):
        return filesystem_dataset_locator()
    return next(
        (
            str(value)
    ...
    for cluster in clusters:
        ...
        if manual_conversation_id is not None:
            ...
            conversation_id = upsert_conversation_row(
                ...
                source_locator=pst_chat_conversation_source_locator(cluster),
...
def build_conversation_preview_head_html() -> str:
    return (
        ...
        "body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; margin: 0; background: linear-gradient(180deg, #eef3f8 0%, #f7fafc 100%); color: #122033; overflow-wrap: anywhere; }"
        "main { width: 100%; max-width: 1040px; margin: 0 auto; padding: clamp(12px, 2.6vw, 28px) clamp(8px, 2vw, 20px) clamp(24px, 4vw, 44px); }"
        "body > h1 { margin: 0.75rem 0 0.5rem; padding: 0 0.75rem; line-height: 1.2; overflow-wrap: anywhere; }"
        "body > table { border-collapse: collapse; table-layout: fixed; width: calc(100% - 1.5rem); margin: 0 0.75rem 1.25rem; background: rgba(255,255,255,0.88); border: 1px solid #d7e0ea; border-radius: 16px; overflow: hidden; }"
        "body > table th, body > table td { text-align: left; padding: 0.55rem 0.75rem; border-bottom: 1px solid #e3e8ef; vertical-align: top; }"
        "body > table th, body > table td { overflow-wrap: anywhere; }"
        "body > table th { width: min(12rem, 38%); color: #516072; font-weight: 600; }"
        ".conversation-nav, .conversation-segments { display: grid; gap: 0.9rem; }"
        ".conversation-nav, .conversation-segments, .conversation-full-segments, .conversation-segment-group, .conversation-chat-transcript, .conversation-document-body, .conversation-raw-text, .retriever-calendar-invites { min-width: 0; max-width: 100%; }"
        ".conversation-segment-card, .conversation-document { min-width: 0; max-width: 100%; background: rgba(255,255,255,0.94); border: 1px solid #d7e0ea; border-radius: 18px; box-shadow: 0 10px 28px rgba(15, 23, 42, 0.06); }"
        ".conversation-segment-card { padding: 1rem 1.1rem; }"
        ".conversation-segment-card h2 { margin: 0 0 0.35rem; font-size: 1.05rem; overflow-wrap: anywhere; }"
        ...
        ".conversation-segment-banner { min-width: 0; max-width: 100%; background: rgba(255,255,255,0.9); border: 1px solid #d7e0ea; border-radius: 18px; padding: 1rem 1.1rem; box-shadow: 0 10px 28px rgba(15, 23, 42, 0.04); }"
        ...
        ".conversation-document-header { display: flex; flex-wrap: wrap; justify-content: space-between; gap: 1rem; align-items: flex-start; min-width: 0; margin-bottom: 0.9rem; }"
        ".conversation-document-header > div { min-width: 0; max-width: 100%; }"
        ".conversation-document-header h2 { margin: 0.2rem 0 0; font-size: 1.15rem; overflow-wrap: anywhere; }"
        ".conversation-document-kind { font-size: 0.82rem; letter-spacing: 0.06em; text-transform: uppercase; color: #516072; }"
        ".conversation-document-meta { display: grid; grid-template-columns: repeat(auto-fit, minmax(min(220px, 100%), 1fr)); gap: 0.7rem 1rem; margin: 0 0 1rem; }"
        ".conversation-document-meta div { min-width: 0; background: #f8fafc; border: 1px solid #e3e8ef; border-radius: 14px; padding: 0.65rem 0.75rem; }"
        ".conversation-document-meta dt { font-size: 0.8rem; font-weight: 600; color: #607080; margin-bottom: 0.18rem; }"
        ".conversation-document-meta dd { margin: 0; overflow-wrap: anywhere; }"
        ".retriever-calendar-invites { margin: 0 0 1rem; display: grid; gap: 0.8rem; }"
        ...
        ".retriever-calendar-invite-header { display: flex; flex-wrap: wrap; justify-content: space-between; gap: 0.75rem; align-items: flex-start; min-width: 0; }"
        ".retriever-calendar-invite-kicker { margin: 0 0 0.25rem; color: #4a6488; font-size: 0.76rem; font-weight: 700; letter-spacing: 0.06em; text-transform: uppercase; }"
        ".retriever-calendar-invite-title { margin: 0; font-size: 1rem; line-height: 1.3; }"
        ".retriever-calendar-invite-title a { color: #174ea6; text-decoration: none; }"
        ".retriever-calendar-invite-title a:hover { text-decoration: underline; }"
        ".retriever-calendar-invite-detail { margin: 0; color: #5f6368; font-size: 0.84rem; white-space: normal; }"
        ".retriever-calendar-invite-meta { display: grid; grid-template-columns: repeat(auto-fit, minmax(min(220px, 100%), 1fr)); gap: 0.6rem 0.75rem; margin: 0.85rem 0 0; }"
        ".retriever-calendar-invite-meta div { min-width: 0; background: rgba(255,255,255,0.82); border: 1px solid #d7e6ff; border-radius: 12px; padding: 0.6rem 0.7rem; }"
        ".retriever-calendar-invite-meta dt { font-size: 0.78rem; font-weight: 700; color: #516072; margin-bottom: 0.18rem; letter-spacing: 0.04em; text-transform: uppercase; }"
        ".retriever-calendar-invite-meta dd { margin: 0; }"
        ".retriever-calendar-invite-meta a { color: #174ea6; text-decoration: none; word-break: break-all; }"
        ".retriever-calendar-invite-meta a:hover { text-decoration: underline; }"
        ...
        ".conversation-chat-body { white-space: pre-wrap; line-height: 1.45; font-size: 1rem; word-break: break-word; overflow-wrap: anywhere; }"
        ...
        "body > table, body > table tbody, body > table tr, body > table th, body > table td { display: block; width: 100%; }"
        "body > table th { padding-bottom: 0.15rem; }"
        "body > table td { padding-top: 0; }"
        ".conversation-document, .conversation-segment-card, .conversation-segment-banner { border-radius: 12px; padding: 0.85rem; }"
        ".conversation-chat-message { gap: 0.55rem; padding: 0.7rem; }"
    ...
    *,
    ...
    cursor = connection.execute(
        ...
        WHERE lifecycle_status != 'deleted'
          AND (id IN ({placeholders}) OR parent_occurrence_id IN ({placeholders}))
        """,
        [seen_at, seen_at, *root_occurrence_ids, *root_occurrence_ids],
    )
    affected_document_ids = container_document_ids_for_root_occurrence_ids(connection, root_occurrence_ids)
...
def assign_dataset_to_container_documents(
    ...
    *,
    ...
    _ = (dataset_id, dataset_source_id)
...
def existing_container_entries_by_source_item(
    connection: sqlite3.Connection,
    *,
    source_kind: str,
    source_rel_path: str,
    ...
    if not occurrence_rows:
        return {}
    document_ids = sorted({int(row["document_id"]) for row in occurrence_rows})
    ...
    rows_by_source_item: dict[str, list[sqlite3.Row]] = defaultdict(list)
    for occurrence_row in occurrence_rows:
        document_id = int(occurrence_row["document_id"])
        if document_id not in documents_by_id:
            continue
        rows_by_source_item[str(occurrence_row["source_item_id"])].append(occurrence_row)
    entries: dict[str, dict[str, sqlite3.Row]] = {}
    for source_item_id, source_rows in rows_by_source_item.items():
        preferred_row = select_preferred_occurrence(source_rows) or source_rows[0]
        entries[source_item_id] = {
            "document_row": documents_by_id[int(preferred_row["document_id"])],
            "occurrence_row": preferred_row,
        }
    return entries


def delete_documents_with_only_deleted_occurrences(
    connection: sqlite3.Connection,
    paths: dict[str, Path],
...
def retire_unseen_container_messages(
    ...
    *,
    ...
    for document_id in sorted(affected_document_ids):
        refresh_source_backed_dataset_memberships_for_document(connection, document_id)
        refresh_document_from_occurrences(connection, document_id)
    delete_documents_with_only_deleted_occurrences(
        connection,
        paths,
        affected_document_ids,
        deleted_at=now,
    )
    return len(root_occurrence_rows)
    ...
    for source_row in source_rows:
        ...
        documents_missing += sum(
            ...
            if connection.execute(
                "SELECT lifecycle_status FROM documents WHERE id = ?",
                (document_id,),
            ).fetchone()["lifecycle_status"] == "missing"
        )
...
def commit_prepared_container_message(
    ...
    *,
    ...
    extracted_payload = dict(prepared_item["extracted_payload"] or {})
    resolved_custodian = normalize_whitespace(str(extracted_payload.get("custodian") or "")) or None
    if resolved_custodian is None:
        resolved_custodian = infer_source_custodian(
            source_kind=source_kind,
            source_rel_path=source_rel_path,
        )
    ...
    *,
    ...
    for prepared_item, wait_ms in iter_prepared_container_message_items(
        ...
        if prepared_item.get("skip"):
            continue
        message_count += 1
        existing_entry = existing_entries_by_source_item.get(str(prepared_item["source_item_id"]))
        existing_row = (
            existing_entry["document_row"]
            if isinstance(existing_entry, dict)
            else None
        )
        existing_occurrence_row = (
            existing_entry["occurrence_row"]
            if isinstance(existing_entry, dict)
            else None
        )
