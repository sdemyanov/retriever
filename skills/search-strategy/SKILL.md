...
- Treat "show", "show me", "list", "display", "browse", "which documents", and similar listing verbs as requests for the standard `/search` table output, not as permission to improvise a custom list format.
- When the user asked to show/list/display/browse documents or to see the table itself, prefer the slash/session browse surface so the saved `/page-size`, `/columns`, and `/sort` state apply, then forward the tool's `rendered_markdown` as the entire reply. Do not add any text before or after the table.
- Do not exceed the active `/page-size` when showing document rows unless the user explicitly asks for more or changes the page size.
...
- When the user asks to show files, documents, or attachment children, make every shown result clickable with its preview/open link and keep the standard `/search` schema unless the user asked for another layout.
