...
SLASH_USE_WHEN: dict[str, str] = {
    ...
    "export": "the user asks to export the current scope/results as a table or archive, or asks export status — `/export table` starts a bounded CSV/table export, `/export archive` starts a bounded zip export, and `/export status` inspects resumable exports; `/export previews` is intentionally deferred until preview export is resumable",
